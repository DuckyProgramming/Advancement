...
class troop extends physical{
    ...
    setupStats(){
        ...
        this.timer={life:types.troop[this.type].timer.life,shield:0}
        this.base.primary={reload:types.primary[this.primaryKey].reload}
        this.base.secondary={reload:types.secondary[this.secondaryKey].reload}
        this.base.timer={life:this.timer.life,shield:0}
            ...
            if(this.team==game.player.team){
                if(this.control==0){
                    ...
                    for(let a=0,la=entities.troops.length;a<la;a++){
                        if(dist(this.position.x,this.position.y,entities.troops[a].position.x,entities.troops[a].position.y)<this.size+entities.troops[a].size&&entities.troops[a].team==this.team&&entities.troops[a].life>0){
                            this.life=1
